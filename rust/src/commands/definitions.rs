    ...
    vec![
        ...
        CreateCommand::new("tickets")
            ...
            .add_option(sub("addtype", "Add or update a ticket type")
                ...
                .add_sub_option(opt(CommandOptionType::Role, "support_role", "Role that handles this type (defaults to the mod role)"))
                .add_sub_option(opt(CommandOptionType::Channel, "category", "Category this type's tickets open under")
            ...
            .add_option(sub("support", "Add or remove a support role on one ticket type")
                .add_sub_option(add_remove("action", "add or remove"))
                .add_sub_option(req(CommandOptionType::String, "key", "The type's key"))
                .add_sub_option(req(CommandOptionType::Role, "role", "Role that handles this type")))
            .add_option(sub("typecategory", "Set the category one ticket type opens under")
                .add_sub_option(req(CommandOptionType::String, "key", "The type's key"))
            ...
            .add_option(sub("category", "Set the category new ticket channels are created under")
                .add_sub_option(req(CommandOptionType::Channel, "category", "Category channel")
                    .channel_types(vec![ChannelType::Category])))
            .add_option(sub("panel", "Post or refresh a ticket panel")
                ...
                .add_sub_option(opt(CommandOptionType::String, "key", "Put just this one type on its own panel here"))),
        ...
        CreateCommand::new("applications")
            ...
            .add_option(sub("setoutcome", "Set where accepted and denied applications get filed")
                .add_sub_option(req(CommandOptionType::String, "key", "The application's key"))
                .add_sub_option(opt(CommandOptionType::Channel, "accepted", "Channel for accepted applications").channel_types(vec![ChannelType::Text]))
                .add_sub_option(opt(CommandOptionType::Channel, "denied", "Channel for denied applications").channel_types(vec![ChannelType::Text])))
