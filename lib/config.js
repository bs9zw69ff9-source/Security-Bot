...
const config = {
  ...
  nukeChannelCreateThresh:  parseInt(process.env.NUKE_CHANNEL_CREATE_THRESH) || 3,
  nukeRoleThreshold:        parseInt(process.env.NUKE_ROLE_THRESHOLD)        || 3,
  nukeRoleCreateThresh:     parseInt(process.env.NUKE_ROLE_CREATE_THRESH)    || 3,
  nukeBanThreshold:         parseInt(process.env.NUKE_BAN_THRESHOLD)         || 3,
  nukeKickThreshold:        parseInt(process.env.NUKE_KICK_THRESHOLD)        || 3,
  ...
  nukeEmojiThreshold:       parseInt(process.env.NUKE_EMOJI_THRESHOLD)       || 3,
  // Shared across EVERY destructive action, so a nuke spread across categories
  // can't hide under each individual threshold. 0 disables the aggregate check.
  nukeTotalThreshold:       parseInt(process.env.NUKE_TOTAL_THRESHOLD ?? "3"),
